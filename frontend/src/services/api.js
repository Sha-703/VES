...
// Force API base URL to the deployed backend domain.
// NOTE: This hardcodes the backend URL; if you later deploy the backend to a
// different domain, update this value or switch back to using `VITE_API_BASE_URL`.
// `API_BASE` includes the `/api/` path and is used for axios requests.
// `API_HOST` is the bare host (no `/api/`) and is exported for building absolute
// URLs to media/static files returned by the backend (e.g. candidate.photo).
const API_HOST = 'https://ves-mg2a.onrender.com';
